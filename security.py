...
def refresh_token(data: dict, expires_delta: timedelta | None = None) -> str:
     to_encode = data.copy()
     if expires_delta:
          expire = datetime.now(timezone.utc) + expires_delta
     else:
     ...
     to_encode.update({"exp": expire})
     refreshed_token = jwt.encode(to_encode, settings.SECRET_KEY.get_secret_value(), algorithm="HS256")
